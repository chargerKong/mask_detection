...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        x = torch.sigmoid(prediction[..., 0])  # Center, tx     # (b,3,13,13)                   # 1 +
        y = torch.sigmoid(prediction[..., 1])  # Center, ty     # (b,3,13,13)                   # 1 +
        ...
        # 但是这里依然计算，并在build_targets函数中出现，其目的是为后续评价
        ...
        if targets is None:
            ...
            # 所以此时为了平衡，noobj_scale往往大于obj_scale, (100, 1) || yolov1: (0.5, 5)
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416):
        ...
        # module_defs = [{"type":"net", "channels":3, ...},         # each element is a layer block (dtype=dict)
        #                {"type":"convolutional", "batch_normalize":1, ...},
        #                ...]
